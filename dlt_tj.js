	...
	static validateNumberRange(recommendations) {
			...
			for (let item of recommendations) {
					for (let r of item.red) {
							...
							let n = parseInt(r, 10);
							if (n >= 1 && n <= 35) {
									uniqueReds.add(r); // 收集 1-35 范围内的不重复前区号码
							}
					}
			}
			...
			// 计算不重复前区号码占总前区池(35个)的比例
			let ratio = uniqueReds.size / 35;

			// 大乐透 5 组数据最多 25 个号码，理论极限覆盖率约 71.4%
			// 这里设定一个合理的及格线阈值，例如 35% (即 5 组推荐里至少要有约 12 个以上不同的号码)，设定在0.35到0.45之间比较合适
			let minRatio = 0.45; 
			
			if (ratio < minRatio) {
					return [false, `前区总体覆盖率不足 ${(minRatio * 100).toFixed(0)}% (当前: ${(ratio * 100).toFixed(1)}%)`];
			}
			
			return [true, `前区池覆盖率达标: ${(ratio * 100).toFixed(1)}% (不重复号码数: ${uniqueReds.size}/35)`];
	}
